...
class GamificationEngine {
  ...
  private checkBadges(): void {
    ...
    this.state.modulesCompleted.forEach((module) => {
      if (
        module.includes("post") &&
        !this.state.badges.includes("rights-guardian")
      ) {
        this.unlockBadge("rights-guardian");
      }
      if (
        module.includes("poat") &&
        !this.state.badges.includes("activity-hero")
      ) {
        this.unlockBadge("activity-hero");
      }
      if (
        module.includes("ai") &&
        !this.state.badges.includes("ai-elder-apprentice")
      ) {
        this.unlockBadge("ai-elder-apprentice");
      }
      if (
        module.includes("sdg") &&
        !this.state.badges.includes("sustainability-steward")
      ) {
        this.unlockBadge("sustainability-steward");
      }
      if (
        module.includes("quantum") &&
        !this.state.badges.includes("quantum-defender")
      ) {
  ...
  completeLesson(
    lessonId: string,
    xp: number = XP_RULES.COMPLETE_LESSON,
  ): void {
  ...
  completeModule(moduleId: string): void {
    if (!this.state.modulesCompleted.includes(moduleId)) {
      ...
      if (typeof window !== "undefined") {
        window.dispatchEvent(
          new CustomEvent("module-completed", {
  ...
  async loadFromBackend(userId: string): Promise<void> {
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/v1/users/${userId}/gamification`,
      );
      if (response.ok) {
