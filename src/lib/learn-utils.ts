...
export function getAllLessonFiles(): LessonFile[] {
  ...
  const baseDir = existsSync(join(process.cwd(), "src"))
  ...
  try {
    if (!existsSync(learnDir)) {
      console.error(
        `[Learn Utils] Learn directory does not exist: ${learnDir}`,
      );
      console.error(
        `[Learn Utils] Current working directory: ${process.cwd()}`,
      );
      return lessons;
    }
    ...
    levelDirs.forEach((levelStr) => {
      ...
      const files = readdirSync(levelDir).filter(
...
export function loadLessonById(lessonId: string): LessonMetadata | null {
  try {
    ...
    if (lessonId.includes("-") && /^\d+-\d+$/.test(lessonId)) {
      ...
      if (!mappedSlug) {
        console.error(
          `[Learn Utils] Could not map lesson ID ${lessonId} to slug`,
        );
        return null;
      }
      slug = mappedSlug;
    ...
    if (!lessonFile) {
      console.error(
        `[Learn Utils] Lesson file not found for slug: ${slug} (from ID: ${lessonId})`,
      );
      console.error(
        `[Learn Utils] Available slugs for level ${level || "any"}: ${allFiles
    ...
    if (!existsSync(lessonFile.path)) {
      console.error(
        `[Learn Utils] Lesson file path does not exist: ${lessonFile.path} (slug: ${slug}, ID: ${lessonId})`,
      );
      return null;
    }
...
export function loadLessonBySlug(slug: string): LessonMetadata | null {
  try {
    ...
    if (!lessonFile) {
      ...
      console.error(
        `[Learn Utils] Available slugs: ${allFiles.map((f) => `${f.level}-${f.slug}`).join(", ")}`,
      );
      return null;
    }
    ...
    if (!existsSync(lessonFile.path)) {
      console.error(
        `[Learn Utils] Lesson file path does not exist: ${lessonFile.path}`,
      );
      return null;
    }
 ...
 */
export function listAllLessons(): Array<{
  id: string;
  slug: string;
  level: number;
  title: string;
  ...
  Object.keys(byLevel).forEach((levelStr) => {
    ...
    files.forEach((file, index) => {
      try {
        ...
        console.error(
          `[Learn Utils] Error reading lesson file ${file.path}:`,
          error,
        );
      }
    });
...
export function generateQuizForLesson(lesson: LessonMetadata): {
  ...
  if (questions.length === 0) {
    ...
    questions.push({
      ...
      options: [`${lesson.duration} minutes`, "5 minutes", "1 hour", "2 hours"],
      correct: 0,
    });
  }
