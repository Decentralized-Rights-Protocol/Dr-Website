...
export function GovernanceBoard({ walletAddress }: GovernanceBoardProps) {
  ...
  const onVote = async (
    ...
    try {
      ...
      setMessage(
        `Recorded a ${choice} vote in Convex. Protocol finality remains outside the app layer.`,
      );
    } catch (error) {
      setMessage(
    ...
    <section className="space-y-4">
      ...
      {proposals.length === 0 ? (
        <div className="rounded-3xl border border-dashed border-neutral-300/80 bg-white/60 p-6 text-sm text-neutral-500 dark:border-neutral-700 dark:bg-neutral-900/30 dark:text-neutral-400">
          No live proposals yet. Bootstrap one in Convex or mirror one from
          Dr-Blockchain when the sync bridge is ready.
        </div>
      ) : (
        proposals.map((proposal: any) => (
          <article
            key={proposal._id}
            className="rounded-3xl border border-neutral-200/80 bg-white/90 p-6 shadow-sm dark:border-neutral-800/80 dark:bg-neutral-900/60"
          >
            <div className="flex flex-wrap items-start justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-xs text-neutral-500 dark:text-neutral-400">
                  App-layer governance record. Final protocol execution must
                  come from Dr-Blockchain or a mirrored governance event.
                </p>
              </div>
            ...
            <div className="mt-4 grid gap-3 sm:grid-cols-3">
              ...
              <div className="rounded-2xl border border-rose-300/50 bg-rose-50/60 p-4 dark:border-rose-500/40 dark:bg-rose-900/20">
                ...
                <p className="mt-1 text-2xl font-semibold text-rose-800 dark:text-rose-100">
                  {proposal.noWeight}
                </p>
              </div>
              <div className="rounded-2xl border border-slate-300/50 bg-slate-50/60 p-4 dark:border-slate-500/40 dark:bg-slate-900/20">
                <p className="text-xs uppercase tracking-wide text-slate-700 dark:text-slate-200">
                  Abstain
                </p>
