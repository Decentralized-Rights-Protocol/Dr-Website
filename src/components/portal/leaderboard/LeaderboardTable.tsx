  ...
  if (data.length === 0) {
    return (
      <div className="rounded-3xl border border-amber-200/70 bg-amber-50/60 p-6 text-sm text-amber-800 dark:border-amber-500/70 dark:bg-amber-900/30 dark:text-amber-200">
        No leaderboard activity yet. Approved submissions and completed learning
        records will populate this table.
      </div>
    );
  }
      ...
      <header className="mb-4 flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-sm text-neutral-500 dark:text-neutral-400">
            Ranked by total verified impact & rewards.
          </p>
        </div>
      </header>
            ...
            {data.map((entry: any) => (
              <tr key={entry.address}>
                ...
                <td className="px-3 py-2 text-neutral-700 dark:text-neutral-200">
                  {entry.totalRewards.toLocaleString()} credits
                </td>
              </tr>
            ))}
          </tbody>
