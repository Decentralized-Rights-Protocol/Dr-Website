...
export function ReviewQueue({ walletAddress }: ReviewQueueProps) {
  ...
  const workspace = useQuery(api.review.listReviewQueue, {
    walletAddress,
    queueStatus: "pending",
  });
  const updateReview = useMutation(
    api.submissions.updateSubmissionReviewStatus,
  );
  ...
  if (!workspace) {
    return (
      <div className="rounded-3xl border border-neutral-200/80 bg-white/80 p-6 text-sm text-neutral-500 dark:border-neutral-800/80 dark:bg-neutral-900/60 dark:text-neutral-400">
        Loading stewardship review queue…
      </div>
    );
  }
  ...
  if (!workspace.authorized) {
    return (
      <div className="rounded-3xl border border-amber-300/60 bg-amber-50/70 p-6 text-sm text-amber-900 dark:border-amber-500/40 dark:bg-amber-950/20 dark:text-amber-100">
        Steward review access is not enabled for this wallet yet. The interface
        is connected, but role assignment still needs authenticated policy
        controls.
      </div>
    );
  }
        ...
        workspace.items.map((item: any) => (
          ...
          >
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {(["approved", "rejected", "needs_info"] as const).map(
                (nextStatus) => (
                  <button
                    key={nextStatus}
                    onClick={async () => {
                      ...
                      await updateReview({
                        ...
                        reviewNote:
                          note[item._id] ||
                          `Marked ${nextStatus} from the stewardship review workspace.`,
                      });
                    }}
