    ...
    <div className="flex flex-col items-center justify-center gap-3">
      ...
      >
        <circle
          ...
          stroke="currentColor"
          strokeWidth={stroke}
          className="text-gray-100 dark:text-foreground/10"
        />
        <circle
          cx={dim / 2}
          cy={dim / 2}
      ...
      {label && (
        <span className="text-xs text-gray-400 dark:text-foreground/30 tracking-widest uppercase animate-pulse">
          {label}
        </span>
      )}
    </div>
...
export function DRPSkeleton({ className = "" }: { className?: string }) {
  return (
    <div
      className={`relative overflow-hidden bg-gray-100 dark:bg-white/5 ${className}`}
    >
      <div
        className="absolute inset-0 -translate-x-full animate-[shimmer_1.5s_infinite]"
        style={{
          background:
            "linear-gradient(90deg, transparent, rgba(0,229,204,0.06), transparent)",
        }}
      />
    </div>
  );
