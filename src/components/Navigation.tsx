...
function ThemeToggle() {
  ...
  return (
    <button onClick={() => setTheme(dark ? 'light' : 'dark')}
      className="w-8 h-8 flex items-center justify-center text-gray-500 dark:text-white/40 hover:text-[#00e5cc] dark:hover:text-[#00e5cc] border border-transparent hover:border-[#00e5cc]/25 transition-all duration-200"
    ...
    <>
      <nav aria-label="Main navigation"
        className={['fixed top-0 left-0 right-0 z-50 transition-all duration-300',
          scrolled ? 'bg-white/96 dark:bg-[#030308]/95 backdrop-blur-2xl border-b border-black/5 dark:border-white/[0.06] shadow-sm dark:shadow-none' : 'bg-transparent',
        ].join(' ')}>
        <div className="max-w-7xl mx-auto px-5 sm:px-6 h-[60px] flex items-center justify-between gap-4">
              ...
              <div key={l.href} className="relative"
                ...
                <Link href={l.href}
                  className={['px-3.5 py-2 text-sm tracking-wide transition-colors duration-200 flex items-center gap-1',
                    isActive(l.href) ? 'text-[#00e5cc]' : 'text-gray-600 dark:text-white/45 hover:text-gray-900 dark:hover:text-white',
                  ].join(' ')}>
                  {l.label}
                  {(l as any).sub && <ChevronDown className="w-3 h-3 opacity-40" />}
                </Link>
                {(l as any).sub && hoveredDrop === l.label && (
                  <div className="absolute top-full left-0 mt-1 w-56 bg-white dark:bg-[#0a0a14] border border-gray-100 dark:border-white/8 shadow-xl z-50">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="https://app.decentralizedrights.com" target="_blank"
              className="hidden sm:inline-flex items-center gap-1.5 px-4 py-2 text-xs font-bold tracking-widest uppercase bg-[#00e5cc] text-black hover:bg-[#00bfff] transition-all duration-200">
              Launch App <ExternalLink className="w-3 h-3" />
            </Link>
            <button onClick={() => setOpen(!open)}
              className="lg:hidden w-8 h-8 flex items-center justify-center text-gray-600 dark:text-white/50" aria-label="Toggle menu">
        ...
        {open && (
          <div className="lg:hidden bg-white dark:bg-[#030308] border-t border-gray-100 dark:border-white/5 max-h-[80vh] overflow-y-auto">
                  ...
                  {(l as any).sub ? (
                    <>
                      <button onClick={() => setExpandedMobile(expandedMobile === l.label ? null : l.label)}
                        className="w-full flex items-center justify-between py-2.5 text-sm text-gray-700 dark:text-white/50">
                        <span>{l.label}</span>
                      ...
                      {expandedMobile === l.label && (
                        <div className="pl-4 space-y-1 pb-2">
                          {(l as any).sub.map((s: any) => (
                            <Link key={s.href} href={s.href}
                              className="block py-2 text-sm text-gray-500 dark:text-white/35 hover:text-[#00e5cc] transition-colors">
                              {s.label}
                            </Link>
                          ))}
                        </div>
                    ...
                    <Link href={l.href}
                      className={['block py-2.5 text-sm transition-colors',
                        isActive(l.href) ? 'text-[#00e5cc]' : 'text-gray-700 dark:text-white/50 hover:text-[#00e5cc]',
                      ].join(' ')}>
                      {l.label}
                    </Link>
                  )}
