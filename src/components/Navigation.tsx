  ...
  return (
    <nav
      className={[
        ...
        scrolled
          ? "bg-white/96 dark:bg-background/95 backdrop-blur-2xl border-b border-black/5 dark:border-foreground/[0.06] shadow-sm dark:shadow-none"
          : "bg-transparent",
      ].join(" ")}
    >
      <div className="max-w-7xl mx-auto px-5 sm:px-6 h-[60px] flex items-center justify-between gap-4">
        <Link href="/" className="flex items-center gap-2.5 group shrink-0">
          <Image
            ...
            priority
          />
          <span className="font-bold tracking-[0.2em] text-xs uppercase hidden sm:block text-gray-900 dark:text-foreground">
            DRP
          </span>
        </Link>
            ...
            >
              <Link
                ...
                className={[
                  ...
                  isActive(l.href)
                    ? "text-[#00e5cc]"
                    : "text-gray-600 dark:text-foreground/45 hover:text-gray-900 dark:hover:text-foreground",
                ].join(" ")}
              >
                {l.label}
                {(l as any).sub && (
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="lg:hidden w-8 h-8 flex items-center justify-center text-gray-600 dark:text-foreground/50"
                ...
                {(l as any).sub ? (
                  <>
                    <button
                      onClick={() =>
                        setExpandedMobile(
                          expandedMobile === l.label ? null : l.label,
                        )
                      }
                    ...
                    >
                      <span>{l.label}</span>
                      <ChevronDown
                        className={`w-4 h-4 opacity-40 transition-transform ${expandedMobile === l.label ? "rotate-180" : ""}`}
                      />
                    </button>
                      ...
                      <div className="pl-4 space-y-1 pb-2">
                        {(l as any).sub.map((s: any) => (
                          <Link
                            key={s.href}
                            href={s.href}
                            className="block py-2 text-sm text-gray-500 dark:text-foreground/35 hover:text-[#00e5cc]"
                          >
                            {s.label}
                          </Link>
                        ))}
                  ...
                  <Link
                    ...
                    className={[
                      ...
                      isActive(l.href)
                        ? "text-[#00e5cc]"
                        : "text-gray-700 dark:text-foreground/50 hover:text-[#00e5cc]",
                    ].join(" ")}
                  >
                    {l.label}
                  </Link>
