...
function ThemeToggle() {
  ...
  return (
    <button onClick={() => setTheme(dark ? 'light' : 'dark')}
      className="w-8 h-8 flex items-center justify-center text-gray-500 dark:text-white/40 hover:text-[#00e5cc] dark:hover:text-[#00e5cc] border border-transparent hover:border-[#00e5cc]/25 transition-all"
    ...
    <nav className={['fixed top-0 left-0 right-0 z-50 transition-all duration-300',
      ...
      <div className="max-w-7xl mx-auto px-5 sm:px-6 h-[60px] flex items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={() => setOpen(!open)} className="lg:hidden w-8 h-8 flex items-center justify-center text-gray-600 dark:text-white/50" aria-label="Toggle menu">
                  ...
                  <>
                    ...
                    {expandedMobile === l.label && (
                      <div className="pl-4 space-y-1 pb-2">
                        {(l as any).sub.map((s: any) => (
                          <Link key={s.href} href={s.href} className="block py-2 text-sm text-gray-500 dark:text-white/35 hover:text-[#00e5cc]">{s.label}</Link>
                        ))}
                      </div>
                    )}
                  </>
                ) : (
                  <Link href={l.href} className={['block py-2.5 text-sm transition-colors', isActive(l.href) ? 'text-[#00e5cc]' : 'text-gray-700 dark:text-white/50 hover:text-[#00e5cc]'].join(' ')}>{l.label}</Link>
                )}
              </div>
            ))}
