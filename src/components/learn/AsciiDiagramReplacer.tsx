...
export const diagramComponents = {
  ...
  code: ({ node, inline, className, children, ...props }: any) => {
    ...
    if (hasBoxDrawing) {
      ...
      let diagramType: "flow" | "layers" | "architecture" | "pipeline" | null =
        null;
      ...
      // Use [\s\S] instead of . with s flag for cross-line matching
      if (
        /┌[\s\S]*?┐[\s\S]*?├[\s\S]*?┤/.test(codeString) ||
        /Layer|Architecture|Stack/i.test(codeString)
      ) {
        diagramType = "layers";
        title = "System Architecture";
        caption =
          "This diagram shows the layered structure described in the text";
      }
      // Check for flow patterns (arrows, sequential steps)
      else if (
        /→|↓|Flow|Process|Step/i.test(codeString) ||
        /Block.*→|→.*Block/.test(codeString)
      ) {
        diagramType = "flow";
        title = "Process Flow";
