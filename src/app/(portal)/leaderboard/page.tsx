...
function RankRow({ entry, rank }: { entry: any; rank: number }) {
  const isTop3 = rank <= 3
  return (
    <div className={`flex items-center gap-4 p-4 border-b border-gray-100 dark:border-foreground/5 ${isTop3 ? 'bg-[#00e5cc]/[0.02]' : ''}`}>
      <span className={`text-sm font-black w-8 text-center ${rank === 1 ? 'text-[#ffd700]' : rank === 2 ? 'text-gray-400' : rank === 3 ? 'text-amber-600' : 'text-gray-400 dark:text-foreground/25'}`}>#{rank}</span>
      ...
      <div className="flex-1 min-w-0">
        <p className="text-sm font-semibold text-foreground truncate">{entry.displayName || (entry.address ? entry.address.slice(0, 12) + '...' : 'Anonymous')}</p>
        <p className="text-xs text-gray-400 dark:text-foreground/25 font-mono">{entry.address?.slice(0, 8)}...{entry.address?.slice(-4)}</p>
      </div>
      <div className="flex items-center gap-1.5 shrink-0">
        <Star className="w-3.5 h-3.5 text-[#ffd700]" />
        <span className="text-sm font-bold text-foreground">{entry.impactScore?.toFixed(0) ?? 0}</span>
      </div>
    </div>
  )
}
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-500 dark:text-foreground/40">Top DRP participants by activity score</p>
      </div>
