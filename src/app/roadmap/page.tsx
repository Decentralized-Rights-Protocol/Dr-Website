        ...
        <div className="relative">
          ...
          <div className="space-y-12">
            {roadmapItems.map((item) => (
              ...
              >
                <div
                  className={`absolute left-0 top-5 flex h-6 w-6 items-center justify-center rounded-full border border-foreground/15 ${item.bgColor}`}
                >
                  <item.icon className={`h-3.5 w-3.5 ${item.color}`} />
                </div>
                <article className="ml-12 flex-1 rounded-2xl border border-foreground/10 bg-black/30 p-6">
                  <div
                    className={`inline-flex items-center rounded-full px-3 py-1 text-xs font-semibold ${item.bgColor} ${item.color}`}
                  >
                  ...
                  <div className="mt-5 grid gap-3 md:grid-cols-2">
                    {item.features.map((feature) => (
                      <div
                        key={feature}
                        className="rounded-xl border border-foreground/10 bg-white/[0.02] px-3 py-2 text-sm text-slate-300"
                      >
                        {feature}
                      </div>
                    ))}
      ...
      >
        <div className="grid gap-4 md:grid-cols-2">
          {upcomingFeatures.map((feature) => (
            <article
              key={feature.title}
              className="rounded-2xl border border-foreground/10 bg-black/30 p-6"
            >
        ...
        <div className="mx-auto w-full max-w-7xl px-6 lg:px-8">
          <div className="rounded-3xl border border-cyan-300/20 bg-gradient-to-r from-cyan-300/15 via-blue-300/10 to-indigo-300/10 p-8 text-center">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-slate-300">
              Our roadmap is intentionally collaborative. Join the community and
