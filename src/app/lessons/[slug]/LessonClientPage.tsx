      ...
      <div className="flex flex-wrap items-center justify-center gap-4 min-w-max mx-auto px-4 py-6">
        {nodes.map((node: any, i: number) => (
          <div key={node.id} className="flex items-center gap-4">
            ...
            >
              <div className="font-bold text-foreground text-sm whitespace-pre-line leading-tight">
                {node.label}
              </div>
              {node.sublabel && (
                <div className="text-xs mt-1.5 text-drp-gray/80 whitespace-pre-line leading-relaxed font-mono">
                  {node.sublabel}
                </div>
              )}
            </motion.div>
            {i < nodes.length - 1 && (
              <div className="flex flex-col items-center gap-1 text-drp-cyan/40 shrink-0">
                ...
                {edges[i]?.label && (
                  <div className="text-[9px] font-cinematic uppercase tracking-widest max-w-[60px] text-center leading-tight">
                    {edges[i].label}
                  </div>
                )}
              </div>
          ...
          <thead>
            <tr className="bg-foreground/5">
              {data.columns.map((col: string, i: number) => (
                <th
                  key={i}
                  className="px-6 py-4 text-left text-drp-gray font-bold uppercase tracking-widest text-xs border-b border-foreground/5"
                >
                  {col}
                </th>
              ))}
              ...
              >
                {row.map((cell: string, ci: number) => (
                  <td
                    ...
                    className={cn(
                      "px-6 py-4",
                      ci === 0 ? "font-bold text-drp-cyan" : "text-drp-gray/90",
                    )}
                  >
      ...
      {[data.left, data.right].filter(Boolean).map((side: any, i: number) => (
        ...
        >
          ...
          <div className="flex items-center gap-3 mb-4">
            <div className="w-8 h-8 rounded-xl flex items-center justify-center text-lg bg-card/40 border border-foreground/10">
              {side.icon}
            </div>
          ...
          <ul className="space-y-3">
            {side.points.map((pt: string, pi: number) => (
              <li
                key={pi}
                className="flex items-start gap-3 text-sm text-drp-gray/80 leading-relaxed"
              >
                <span
      ...
      {[...data.layers].reverse().map((layer: any, i: number) => (
        ...
        >
          <div className="flex items-center gap-4 mb-3">
            <div className="w-10 h-10 rounded-xl flex items-center justify-center text-xl bg-card/40 border border-foreground/10">
              {layer.icon}
            </div>
            <div>
              ...
              <div className="text-xs text-drp-gray font-cinematic uppercase tracking-[0.2em]">
                {layer.description}
              </div>
            </div>
          </div>
          <div className="flex flex-wrap gap-2">
            {layer.items.map((item: string, ii: number) => (
              <span
                key={ii}
                className="px-3 py-1 rounded-lg text-xs font-mono font-bold text-foreground/70 border border-foreground/5 bg-card/40"
              >
                {item}
              </span>
            ))}
        ...
        >
          ...
          <div className="flex flex-col items-center">
            <div
              className={cn(
                ...
                event.status === "active"
                  ? "bg-drp-cyan text-background border-drp-cyan shadow-[0_0_20px_rgba(0,242,255,0.3)]"
                  : "bg-card/40 text-foreground/40 border-foreground/10",
              )}
            >
              {event.phase}
            </div>
          ...
          <div className="flex-1 pb-12">
            <div className="flex items-center gap-3 mb-2">
              ...
              <span className="px-2 py-0.5 rounded-full bg-foreground/5 border border-foreground/10 text-[9px] font-bold text-drp-gray uppercase tracking-widest">
                {event.date}
              </span>
            </div>
            <div className="grid sm:grid-cols-2 gap-x-8 gap-y-2">
              {event.items.map((item: string, ii: number) => (
                <div
                  key={ii}
                  className="flex items-start gap-2 text-sm text-drp-gray/80"
                >
    ...
    <div className="my-8 rounded-3xl overflow-hidden border border-foreground/10 bg-black/60 shadow-2xl group">
      <div className="flex items-center justify-between px-6 py-3 bg-foreground/5 border-b border-foreground/10">
        <div className="flex items-center gap-3">
          ...
          {section.title && (
            <span className="text-xs font-bold text-drp-gray uppercase tracking-widest">
              {section.title}
            </span>
          )}
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[9px] font-mono text-drp-cyan font-bold uppercase px-2 py-0.5 rounded bg-drp-cyan/10 border border-drp-cyan/20">
            {section.language || "code"}
          </span>
          <button
            onClick={copy}
            className="text-[9px] font-bold text-foreground/40 hover:text-drp-cyan transition-colors uppercase tracking-widest"
          >
            {copied ? "Copied ✓" : "Copy"}
          </button>
        </div>
        ...
        <code className="text-drp-gray/90">
          {section.code?.split("\n").map((line, i) => (
            <div key={i} className="flex gap-4">
              <span className="text-foreground/10 select-none w-4 text-right">
                {i + 1}
              </span>
              <span>{line}</span>
            </div>
...
function CalloutBlock({ section }: { section: LessonSection }) {
  ...
  return (
    ...
    >
      ...
      <p className="text-drp-gray leading-relaxed text-sm md:text-base">
        {section.content}
      </p>
    </div>
  );
      ...
      <div className="px-8 py-6 border-b border-foreground/10 bg-foreground/5 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs font-cinematic text-drp-cyan tracking-widest uppercase opacity-60">
              Verification Phase
            </p>
          </div>
        </div>
        <div className="hidden sm:block">
          <span className="px-3 py-1 rounded-full text-xs font-bold bg-foreground/5 text-foreground/40 uppercase tracking-widest border border-foreground/10">
            Quiz
          </span>
        </div>
      </div>
      <div className="p-8">
        <p className="text-foreground text-lg font-bold mb-8 leading-tight">
          {section.content}
        </p>
        <div className="grid gap-4">
          {(section.quizOptions || []).map((opt, i) => {
            let stateStyle =
              "border-foreground/5 bg-foreground/5 text-drp-gray hover:border-white/20 hover:bg-white/10";
            if (revealed) {
              if (i === correct)
                stateStyle =
                  "border-emerald-500/50 bg-emerald-500/10 text-emerald-400";
              else if (i === selected)
                stateStyle = "border-rose-500/50 bg-rose-500/10 text-rose-400";
              else
                stateStyle =
                  "opacity-40 border-foreground/5 bg-foreground/5 text-drp-gray";
            } else if (selected === i) {
            ...
            return (
              ...
              >
                ...
                {opt}
                {revealed && i === correct && (
                  <CheckCircle2 className="h-5 w-5 ml-auto text-emerald-400" />
                )}
              </button>
            );
          })}
        ...
        <AnimatePresence>
          {!revealed && selected !== null && (
            <motion.button
              ...
              onClick={() => {
                setRevealed(true);
                if (selected === correct && onComplete) onComplete();
              }}
...
function AITutor({ lesson }: { lesson: Lesson }) {
  ...
  >([
    {
      role: "ai",
      text: `Greetings, Steward. I am the DRP Knowledge Core. How can I assist your study of **${lesson.title}**?`,
    },
  ]);
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { role: "ai", text: data.answer || "Analyzing DRP datasets..." },
      ]);
    } catch {
      setMessages((prev) => [
        ...prev,
        {
          role: "ai",
          text: "Connection to Knowledge Core interrupted. Please retry.",
        },
      ]);
    }
    setLoading(false);
                ...
                <div className="flex flex-wrap gap-2">
                  {[
                    ...
                    <button
                      key={h}
                      onClick={() => setInput(h)}
                      className="text-xs font-bold px-3 py-1.5 rounded-full bg-foreground/5 border border-foreground/10 text-foreground/40 hover:text-drp-cyan hover:border-drp-cyan/40 transition-all uppercase tracking-widest"
                    >
                      {h}
                    </button>
                  ))}
...
function LoadingSpinner() {
  return (
    <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
      ...
      <p className="text-sm font-cinematic text-drp-cyan tracking-widest uppercase">
        Initializing Module...
      </p>
    </div>
  );
    ...
    <div className="min-h-screen bg-background text-foreground selection:bg-drp-cyan/30 selection:text-drp-cyan">
      ...
      <div className="sticky top-0 z-[100] border-b border-foreground/5 bg-background/60 backdrop-blur-2xl">
        <div className="max-w-6xl mx-auto px-6 py-4 flex items-center gap-6">
          <Link
            href="/learn"
            className="p-2 rounded-xl hover:bg-foreground/5 text-drp-gray hover:text-foreground transition-all"
          >
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs font-bold text-drp-cyan uppercase tracking-widest">
          ...
          <div className="hidden md:flex items-center gap-6 shrink-0">
            <div className="flex flex-col items-end">
              ...
              <span className="text-xs font-cinematic text-amber-400/40 uppercase tracking-widest">
                Rewards
              </span>
            </div>
            <div className="w-px h-8 bg-foreground/10" />
            <div className="flex flex-col items-end">
              <span className="text-xs font-bold text-drp-cyan">
                +{lesson.xp} XP
              </span>
              <span className="text-xs font-cinematic text-drp-cyan/40 uppercase tracking-widest">
                Experience
              </span>
            </div>
          </div>
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-3 mt-10">
            {lesson.tags.map((tag) => (
              <span
                key={tag}
                className="px-3 py-1 rounded-full text-xs font-bold bg-foreground/5 text-foreground/40 border border-foreground/5 uppercase tracking-widest"
              >
                {tag}
              </span>
            ))}
            ...
            >
              {section.type === "intro" && (
                <div className="py-12 border-l-2 border-foreground/5 pl-8 md:pl-12 my-8">
                  ...
                  <button
                    onClick={() => mark(i)}
                    className="mt-8 flex items-center gap-2 text-xs font-bold text-drp-cyan uppercase tracking-widest group"
                  >
                    <div
                      className={cn(
                        "w-5 h-5 rounded-full border border-drp-cyan/30 flex items-center justify-center group-hover:bg-drp-cyan/10",
                        completed.has(i) &&
                          "bg-drp-cyan text-background border-drp-cyan",
                      )}
                    >
                ...
                <div className="my-10 p-10 rounded-[2.5rem] border border-foreground/5 bg-foreground/5 backdrop-blur-xl group hover:border-foreground/20 transition-all">
                  {section.title && (
                    <div className="flex items-center justify-between mb-8">
                      <h3 className="text-2xl font-bold text-foreground tracking-tight flex items-center gap-4">
                        <span className="text-drp-cyan/20 font-black text-4xl">
                          0{i + 1}
                        </span>
                        {section.title}
                      </h3>
                  ...
                  <div className="mt-10 pt-8 border-t border-foreground/5 flex justify-end">
                    ...
                    >
                      {completed.has(i) ? (
                        <>
                          <CheckCircle2 className="h-4 w-4" /> Concept Mastered
                        </>
                      ) : (
              ...
              {section.type === "diagram" && (
                <div className="my-12">
                  ...
                  <div className="rounded-[2.5rem] border border-foreground/5 bg-card/40 p-2 overflow-hidden shadow-2xl">
                    {section.diagramType === "flow" && (
                      <FlowDiagram data={section.diagramData} />
                    )}
                    {section.diagramType === "comparison" && (
                      <ComparisonDiagram data={section.diagramData} />
                    )}
                    {section.diagramType === "stack" && (
                      <StackDiagram data={section.diagramData} />
                    )}
                    {section.diagramType === "timeline" && (
                      <TimelineDiagram data={section.diagramData} />
                    )}
                  </div>
                  {section.content && (
              ...
              {section.type === "quiz" && (
                <QuizBlock section={section} onComplete={() => mark(i)} />
              )}
            </motion.div>
          ))}
        </div>
          ...
          {isFinished && (
            ...
            >
              ...
              <p className="text-drp-gray text-lg mb-10 max-w-xl mx-auto">
                You have successfully completed the protocol verification for
                this module. Your credentials have been updated.
              </p>
              ...
              <div className="flex flex-wrap justify-center gap-8 mb-12">
                <div className="text-center">
                  ...
                  <div className="text-xs font-cinematic text-amber-400/40 uppercase tracking-widest mt-1">
                    $DeRi Tokens
                  </div>
                </div>
                <div className="w-px h-12 bg-foreground/10" />
                <div className="text-center">
                  <div className="text-3xl font-black text-drp-cyan">
                    +{lesson.xp}
                  </div>
                  <div className="text-xs font-cinematic text-drp-cyan/40 uppercase tracking-widest mt-1">
                    Steward XP
                  </div>
                </div>
              </div>
          ...
          <div className="mt-32 pt-12 border-t border-foreground/5 flex items-center justify-between">
            {lesson.prevLesson ? (
              <Link
                ...
                className="flex items-center gap-3 text-drp-gray hover:text-foreground transition-colors group"
              >
                ...
                <div className="text-left">
                  <div className="text-xs font-cinematic uppercase tracking-widest opacity-40">
                    Previous
                  </div>
            ...
            {lesson.nextLesson && (
              <Link
                href={`/lessons/${lesson.nextLesson}`}
                className="flex items-center gap-3 text-drp-gray hover:text-foreground transition-colors group text-right"
              >
