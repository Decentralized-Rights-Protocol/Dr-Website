...
const terms: GlossaryTerm[] = [
  ...
  },
  {
    ...
    definition: (
      <>
        PoST verifies credentials and reputation signals without exposing
        private data, enabling governance weighting and access control based on
        verifiable status.
      </>
    ),
  ...
  },
  {
    ...
    definition: (
      <>
        PoAT records verifiable human activity so rewards and on-chain outcomes
        are grounded in meaningful contributions—not only token behavior.
      </>
    ),
  ...
  },
  {
    ...
    definition: (
      <>
        The set of protocol mechanisms that turn evidence into proofs and proofs
        into audit-ready governance—so AI assistance and blockchain records stay
        explainable.
      </>
    ),
  },
  ...
  return (
    <>
      ...
      <PremiumPage>
        <PremiumHero
          badge="Glossary"
          title="DRP key terms, clearly defined"
          description="Entity-first definitions designed to be easy for humans, search engines, and AI assistants to summarize and cite."
        />
        ...
        >
          <dl className="mt-6 grid gap-4 sm:grid-cols-2">
            {terms.map((t) => (
              <div
                key={t.term}
                className="rounded-2xl border border-foreground/10 bg-white/[0.03] p-6"
              >
