          ...
          {[
            {
              icon: HeartHandshake,
              title: "Human First",
              body: "Technology should enhance dignity and agency, not extract from it.",
            },
            {
              icon: ShieldCheck,
              title: "Trust by Design",
              body: "Security, transparency, and governance are built into system primitives.",
            },
            {
              icon: Sparkles,
              title: "Future-Ready",
              body: "A protocol architecture designed for AI-era and quantum-era realities.",
            },
          ].map((item) => (
            <article
              key={item.title}
              className="rounded-[2rem] border border-foreground/5 bg-white/5 p-8 transition-all hover:bg-white/10"
            >
      ...
      >
        <div className="space-y-4">
          {[
            ...
            <div
              key={principle}
              className="rounded-2xl border border-foreground/5 bg-white/[0.02] px-8 py-4 text-sm text-drp-gray"
            >
              {principle}
            </div>
          ))}
        ...
        <div className="mx-auto w-full max-w-7xl px-6 lg:px-8">
          <div className="rounded-[3rem] border border-foreground/5 bg-gradient-to-br from-drp-cyan/10 via-drp-blue/5 to-transparent p-16 text-center backdrop-blur-xl">
            ...
            <p className="mx-auto max-w-2xl text-lg text-drp-gray mb-12">
              Whether you are a builder, institution, researcher, or advocate,
              DRP is designed for collaborative progress.
            </p>
