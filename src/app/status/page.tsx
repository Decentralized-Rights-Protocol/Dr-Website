  ...
  const checkHealth = async () => {
    ...
    try {
      const rpcUrl =
        ...
        "https://rpc.decentralizedrights.com";
    ...
    try {
      ...
      checks.push({
        service: "OrbitDB",
        status:
          response.ok && data.status === "healthy" ? "healthy" : "degraded",
        latency,
        lastChecked: new Date().toISOString(),
    ...
    try {
      const aiUrl =
        process.env.NEXT_PUBLIC_AI_API || "https://ai.decentralizedrights.com";
      const startTime = Date.now();
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/api/transactions?page=1&page_size=1`,
        {
          method: "GET",
          headers: { "Content-Type": "application/json" },
        },
      ...
      checks.push({
        ...
        details: data.transactions
          ? `${data.total} transactions indexed`
          : "No data",
      });
    } catch (error) {
      checks.push({
        ...
        {loading && healthStatuses.length === 0 ? (
          <div className="space-y-4">
            {[1, 2, 3, 4, 5, 6].map((i) => (
              <div
                key={i}
                className="animate-pulse rounded-3xl border border-neutral-200 bg-white p-6 shadow-sm dark:border-neutral-800 dark:bg-neutral-900 h-24"
              />
            ))}
          </div>
        ) : (
              ...
              >
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between">
                    ...
                    <span
                      className={`px-2 py-1 rounded text-xs font-semibold border capitalize ${getStatusColor(status.status)}`}
                    >
                      {status.status}
                    </span>
                  </div>
            ...
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-600 dark:text-neutral-400 mt-1">
                  Service is operating normally with expected latency
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-neutral-600 dark:text-neutral-400 mt-1">
                  Service is operational but experiencing issues or high latency
                </p>
              </div>
            </div>
            <div className="flex items-start gap-3">
