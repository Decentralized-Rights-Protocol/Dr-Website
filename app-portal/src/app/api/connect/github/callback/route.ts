...
export async function GET(request: NextRequest) {
  ...
  if (!clientId || !clientSecret) return NextResponse.redirect(new URL('/connect?error=github_not_configured', request.url))
  ...
  response.cookies.set('drp_github_token', tokenData.access_token, { httpOnly: true, secure: process.env.NODE_ENV === 'production', sameSite: 'lax', maxAge: 60 * 60 * 24 * 30, path: '/' })
