...
interface ApiRequestOptions<TBody> { path: string; method?: HttpMethod; body?: TBody; headers?: Record<string, string>; isMultipart?: boolean }
...
async function parseResponse<TData>(response: Response): Promise<ApiResponse<TData>> {
  ...
  if (!response.ok) { const error = new Error(`API request failed with status ${response.status}`); ;(error as Error & { payload?: unknown }).payload = payload; throw error }
...
export interface SubmissionResponse { submission_id: string; cid: string; ipfs_cid?: string; status: VerificationStatus | string; timestamp: string; ai_verdict?: string; ai_score?: number }
export interface ActivityClaim { title: string; description: string; location?: string; timestamp: string; media_cid?: string; hash: string; actor_id: string; activity_type?: 'poat' | 'post' }
...
export interface StatusProfile { user_id: string; post_score: number; verified_status: boolean; attestations: Array<{ category: string; issuer: string; verified_at: string; credential_cid: string }>; last_updated: string }
export async function getStatusProfile(userId: string): Promise<StatusProfile> { const response = await apiRequest<StatusProfile>({ path: `/api/status/profile?id=${encodeURIComponent(userId)}` }); return response.data }

export async function submitStatus(claim: StatusClaim): Promise<SubmissionResponse> {
  const body: StatusClaim & { activity_type: 'post' } = { ...claim, activity_type: 'post' }
  const response = await apiRequest<SubmissionResponse, StatusClaim & { activity_type: 'post' }>({ path: '/api/v1/activities/submit', method: 'POST', body })
  return response.data
}
...
export async function getTransactions(params?: { page?: number; page_size?: number; type?: Transaction['type']; status?: Transaction['status'] }): Promise<TransactionsResponse> { const q = new URLSearchParams(); if (params?.page) q.set('page', String(params.page)); if (params?.page_size) q.set('limit', String(params.page_size)); if (params?.type) q.set('type', params.type); if (params?.status) q.set('status', params.status); const response = await apiRequest<Transaction[]>({ path: `/api/v1/explorer/transactions${q.toString() ? `?${q}` : ''}` }); const transactions = response.data || []; return { transactions: transactions.map((tx: any) => ({ tx_hash: tx.tx_hash, block_number: tx.block_number || 0, timestamp: tx.timestamp, from: tx.from_address, to: tx.to_address, value: tx.value, gas_used: tx.gas_used || 0, status: tx.status === 'confirmed' ? 'success' : tx.status === 'failed' ? 'failed' : 'pending', type: tx.type, metadata: tx.metadata })), total: transactions.length, page: params?.page || 1, page_size: params?.page_size || 50 } }
...
export async function getActivityFeed(params?: { page?: number; page_size?: number; actor_id?: string }): Promise<ActivityFeedResponse> { const q = new URLSearchParams(); if (params?.page) q.set('page', String(params.page)); if (params?.page_size) q.set('page_size', String(params.page_size)); if (params?.actor_id) q.set('actor_id', params.actor_id); const response = await apiRequest<ActivityFeedResponse>({ path: `/api/activity/feed${q.toString() ? `?${q}` : ''}` }); return response.data }
