...
Important policy: AI may assist assessment, but it does not independently create
an irreversible rights-bearing attestation or mint rewards. Approved rewards
must follow the protocol's review/attestation path.
"""
...
async def verify_activity_async(
    ...
    try:
        ...
        logger.info(
            "AI assessment queued for review: submission=%s verdict=%s score=%s",
            submission_id,
            assessment.get("verdict"),
            assessment.get("score"),
        )
        # Deliberately no process_reward() call here.
        # A future review/attestation service must establish finality first.
    except Exception:
...
async def get_activity_status(
    ...
    try:
        ...
        return ActivityStatus(
            submission_id=submission_id,
            status=status_data.get("status", "pending_review"),
